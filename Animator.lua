		...
		for Name, posesData in pairs(Data.Poses) do
			if motors[Name] then
				...
				if selfA.Animated ~= false then
					...
					selfA.Tween = TS:Create(motors[Name], TweenInfo.new(timeTOWait, PoseStyleToEasingStyle(posesData.EasingStyle), PoseStyleToEasingDirection(posesData.EasingDirection)), {
						C1 = formula
					})
					selfA.Tween:Play()
				else
...
function module.GenerateKeyframe(KeyFrameSequance)
...
function module.GenerateRig(model)
